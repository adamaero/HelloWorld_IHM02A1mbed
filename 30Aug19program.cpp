...
#define DEG_half (100*128/360)  // Half degree
...
void forward(int motor, int degree)//int motorChoice, int degree
{
    printf("--> Moving forward.\r\n");   
        ...
        else if(motor == 2)
        {
            ...
            positionL1M2 = motors1stLevel[L6470_L1M1]->get_position();
                printf("--> Getting the current position: %d\r\n", positionL1M2);                  
        }                    
        else
        {
            motors2ndLevel[L6470_L2M2]->move(StepperMotor::FWD, DEG_1*degree);
            positionL2M2 = motors1stLevel[L6470_L1M1]->get_position();
                printf("--> Getting the current position: %d\r\n", positionL2M2);      
        }
...
void backward(int motor, int degree)
{
    printf("--> Moving backward.\r\n");
        ...
        {
            motors1stLevel[L6470_L1M1]->move(StepperMotor::BWD, DEG_1*degree);
            positionL1M1 = motors1stLevel[L6470_L1M1]->get_position();
                printf("--> Getting the current position: %d\r\n", positionL1M1);                  
        }
        else if(motor == 2)
        {
            motors1stLevel[L6470_L1M2]->move(StepperMotor::BWD, DEG_1*degree);
            positionL1M2 = motors1stLevel[L6470_L1M1]->get_position();
                printf("--> Getting the current position: %d\r\n", positionL1M2);                  
        }                    
        else
        {
            motors2ndLevel[L6470_L2M2]->move(StepperMotor::BWD, DEG_1*degree);
            positionL2M2 = motors1stLevel[L6470_L1M1]->get_position();
                printf("--> Getting the current position: %d\r\n", positionL2M2);      
        }
...
//    x_nucleo_ihm02a1one = new XNucleoIHM02A1(&init[0], &init[1], A4, A5, D4, A2, &dev_spi);
  ...
  //                                                                                Second shield (3rd motor) does nothing...
    ...
    // Additional commands: //
//          motors[L6470_L1M1]->wait_while_active();
//                        ^ ^
//          wait_ms(DELAY_1);   // 1 second
//          wait_ms(DELAY_2);   // 2 seconds
//          wait_ms(DELAY_3);   // 3 seconds
    // --- --- --- --- ---  //
  ...
  //            [Integers 1, 2 or 4],[+integers]    
    forward(1,360*3);
          wait_ms(DELAY_1);
          motors1stLevel[L6470_L1M1]->wait_while_active();
    backward(2,180);
          wait_ms(DELAY_1);  
          
            
//    forward(4,180);
//          motors2ndLevel[L6470_L1M1]->wait_while_active();
...
// 
//    pc.baud (9600);
//    pc.printf   ("Rotary encoder test sytem starting up\r\n");
// 
//    //  Main loop
//    while(1) 
//    {
